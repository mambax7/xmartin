 ...
 * */
class HotelServiceHandler extends \XoopsPersistableObjectHandler
{
    ...
    public function __construct(\XoopsDatabase $db = null, \XoopsModules\Xmartin\Helper $helper = null)
    {
        ...
        parent::__construct($db, 'xmartin_hotel_service', HotelService::class, 'service_id', 'service_name');
    }
     ...
     *
     * @param \XoopsObject $hotelservice       reference to the {@link hotelservice}
     *                                         object
     * @param bool         $force
    ...
    public function insert(\XoopsObject $hotelservice, $force = false)
    {
        ...
        if ("xoopsmodules\\$moduleDirName\\service" !== mb_strtolower(get_class($hotelservice))) {
            return false;
        }
        ...
        if ($hotelservice->isNew()) {
            $sql = sprintf(
                ...
                $this->db->prefix('xmartin_hotel_service'),
                $service_type_id,
     ...
     * @copyright 1997-2010 The Martin Group
     * @author    Martin <china.codehome@gmail.com>
     * @param \XoopsObject $hotelservice
     * @param bool         $force
     * @return bool|void
     */
    public function delete(\XoopsObject $hotelservice, $force = false)
    {
        ...
        $sql = 'DELETE FROM ' . $this->db->prefix('xmartin_hotel_service') . ' WHERE service_id = ' . $hotelservice->service_id();
    ...
    {
        $sql = 'DELETE FROM ' . $this->db->prefix('xmartin_hotel_service');
    ...
    {
        $sql = 'SELECT COUNT(*) FROM ' . $this->db->prefix('xmartin_hotel_service');
        if (isset($criteria) && $criteria instanceof \CriteriaElement) {
            $sql .= ' ' . $criteria->renderWhere();
        }
        $result = $this->db->query($sql);
    ...
    public function &getObjects(\CriteriaElement $criteria = null, $id_as_key = false, $as_object = true)
    {
        $ret   = [];
        $limit = $start = 0;
        $sql   = 'SELECT s.*,st.service_type_name FROM ' . $this->db->prefix('xmartin_hotel_service') . ' s LEFT JOIN ' . $this->db->prefix('xmartin_hotel_service_type') . ' st ON (s.service_type_id = st.service_type_id ) ';
        if (isset($criteria) && $criteria instanceof \CriteriaElement) {
            $sql .= ' ' . $criteria->renderWhere();
            if ('' != $criteria->getSort()) {
    ...
    {
        $rows   = [];
        $sql    = 'SELECT s.service_name,sr.*,h.hotel_name FROM ' . $this->db->prefix('xmartin_hotel_service_relation') . ' sr
            left join ' . $this->db->prefix('xmartin_hotel_service') . ' s ON (s.service_id = sr.service_id)
            left join ' . $this->db->prefix('xmartin_hotel') . " h ON (sr.hotel_id = h.hotel_id) order BY hotel_id DESC
    ...
    public function getHotelServiceRelation($hotel_id, $service_id)
    {
        ...
        $sql    = 'SELECT s.service_name,sr.*,h.hotel_name FROM ' . $this->db->prefix('xmartin_hotel_service_relation') . ' sr
            left join ' . $this->db->prefix('xmartin_hotel_service') . ' s ON (s.service_id = sr.service_id)
            left join ' . $this->db->prefix('xmartin_hotel') . " h ON (sr.hotel_id = h.hotel_id) WHERE sr.hotel_id = $hotel_id and sr.service_id = $service_id ";
        $result = $this->db->query($sql);
    ...
    public function deleteServiceRelation($hotel_id, $service_id)
    {
        ...
        $sql = 'delete FROM ' . $this->db->prefix('xmartin_hotel_service_relation') . "
    ...
    public function insertRelation($RelationData, $IsOld = false)
    {
        ...
        if (!$IsOld) {
            $sql = 'insert INTO ' . $this->db->prefix('xmartin_hotel_service_relation') . " (`hotel_id`,`service_id`,`service_extra_price`) VALUES ($hotel_id,$service_id,$service_extra_price) ";
        } else {
            $sql = 'UPDATE ' . $this->db->prefix('xmartin_hotel_service_relation') . " SET service_extra_price = $service_extra_price WHERE hotel_id = $hotel_id and service_id = $service_id";
        }
    ...
    public function checkRelationExist($hotel_id, $service_id)
    {
        ...
        $sql = 'SELECT * FROM ' . $this->db->prefix('xmartin_hotel_service_relation') . " WHERE hotel_id = $hotel_id and service_id = $service_id";
    ...
    {
        $rows   = [];
        $sql    = 'SELECT hotel_id ,hotel_name FROM ' . $this->db->prefix('xmartin_hotel');
    ...
    {
        $rows   = [];
        $sql    = 'SELECT service_id ,service_name FROM ' . $this->db->prefix('xmartin_hotel_service');
    ...
    {
        $sql    = 'SELECT COUNT(*) FROM ' . $this->db->prefix('xmartin_hotel_service_relation');
        $result = $this->db->query($sql);
        if (!$result) {
            return 0;
        }
    ...
    public function getHotelService($hotel_id)
    {
        ...
        $DBPrefx = empty($xoopsdModule) ? $xoopsDB->prefix . '_xmartin' : $xoopsDB->prefix . '_' . $xoopsModule->getVar('dirname');
        if (empty($hotel_id)) {
            return false;
        }
