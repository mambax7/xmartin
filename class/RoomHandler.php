 ...
 * */
class RoomHandler extends \XoopsPersistableObjectHandler
{
    /**
     * @var Helper
     */
    ...
    public function __construct(\XoopsDatabase $db = null, \XoopsModules\Xmartin\Helper $helper = null)
    {
        ...
        parent::__construct($db, 'xmartin_room', Room::class, 'room_id', 'room_name');
    }
     ...
     * @return array
     */
    public function &getRooms($limit = 0, $start = 0, $sort = 'room_id', $room = 'ASC', $id_as_key = true)
    {
        $criteria = new \CriteriaCompo();

        $criteria->setSort($sort);
//        $criteria->setroom($room); //TODO
     ...
     *
     * @param \XoopsObject $room       reference to the {@link room}
     *                                 object
     * @param bool         $force
    ...
    public function insert(\XoopsObject $room, $force = false)
    {
        ...
        //        $bingo = "xoopsmodules\\$moduleDirName\\room";
        //        if ($bingo !== strtolower(get_class($room))) {
        if ("xoopsmodules\\$moduleDirName\\room" !== mb_strtolower(get_class($room))) {
            return false;
        }
    ...
    {
        $sql = 'SELECT count(*) AS count FROM ' . $this->db->prefix('xmartin_room') . ' WHERE room_type_id = ' . $roomObj->room_type_id() . ' ';
    ...
    public function delete(\XoopsObject $room, $force = false)
    {
        ...
        $sql = 'DELETE FROM ' . $this->db->prefix('xmartin_room') . ' WHERE room_id = ' . $room->room_id();
    ...
    {
        $sql = 'SELECT COUNT(*) FROM ' . $this->db->prefix('xmartin_room');
    ...
    public function &getObjects(\CriteriaElement $criteria = null, $id_as_key = false, $as_object = true)
    {
        ...
        $sql = 'SELECT r.*,rt.room_type_info,h.hotel_name FROM ' . $this->db->prefix('xmartin_room') . ' r LEFT JOIN ' . $this->db->prefix('xmartin_room_type') . ' rt ON (r.room_type_id = rt.room_type_id ) LEFT JOIN ' . $this->db->prefix('xmartin_hotel') . ' h ON ( h.hotel_id = r.hotel_id ) ';
        if (isset($criteria) && $criteria instanceof \CriteriaElement) {
            $sql .= ' ' . $criteria->renderWhere();
            if ('' != $criteria->getSort()) {
                $sql .= ' room BY ' . $criteria->getSort() . ' ' . $criteria->getroom();
            }
    ...
    {
        $rows   = [];
        $sql    = 'SELECT * FROM ' . $this->db->prefix('xmartin_room_type');
    ...
    {
        $rows   = [];
        $sql    = 'SELECT room_id,room_name FROM ' . $this->db->prefix('xmartin_room');
    ...
    public function insertType($typeData)
    {
        ...
        if ($room_type_id > 0) {
            $sql = 'UPDATE ' . $xoopsDB->prefix('xmartin_room_type') . ' set room_type_info = ' . $xoopsDB->quoteString($room_type_info) . ' WHERE room_type_id = ' . $room_type_id;
        } else {
            $sql = 'insert INTO ' . $xoopsDB->prefix('xmartin_room_type') . " (room_type_id,room_type_info) VALUES (null,'$room_type_info')";
        }
    ...
    public function deleteRoomType($typeid)
    {
        ...
        $sql = 'DELETE FROM ' . $xoopsDB->prefix('xmartin_room_type') . ' WHERE room_type_id  = ' . $typeid;
    ...
    {
        $rows   = [];
        $sql    = 'SELECT rp.*,r.room_name FROM ' . $this->db->prefix('xmartin_room_price');
        $sql    .= ' rp left join ' . $this->db->prefix('xmartin_room') . ' r ON r.room_id = rp.room_id ';
    ...
    {
        $sql = 'SELECT COUNT(*) FROM ' . $this->db->prefix('xmartin_room_price');
        if (isset($criteria) && $criteria instanceof \CriteriaElement) {
            $sql .= ' ' . $criteria->renderWhere();
        }
        $result = $this->db->query($sql);
    ...
    public function truncatePassData($date = 0)
    {
        ...
        $date = strtotime($date);
        $sql  = 'DELETE FROM ' . $xoopsDB->prefix('xmartin_room_price') . ' WHERE room_date < ' . $date;
    ...
    public function deleteRoomPrice($room_id, $room_date)
    {
        ...
        $sql = 'DELETE FROM ' . $xoopsDB->prefix('xmartin_room_price') . ' WHERE room_id  = ' . $room_id . ' AND room_date = ' . strtotime($room_date);
    ...
    public function InsertRoomPrice($Data, $IsOld)
    {
        ...
        global $xoopsDB;
        if ($IsOld) {
            $sql = 'UPDATE ' . $xoopsDB->prefix('xmartin_room_price') . " set
            ...
            $sql    = 'INSERT INTO ' . $xoopsDB->prefix('xmartin_room_price') . ' (room_id,room_is_today_special,room_price,room_advisory_range_small,room_advisory_range_max,room_sented_coupon,room_date ) VALUES ';
            $Insert = false;
            foreach ($Data as $price) {
                if (!$this->checkExistDate($price['room_id'], $price['room_date'])) {
                    ...
                    $upSql = 'UPDATE ' . $xoopsDB->prefix('xmartin_room_price') . " SET
    ...
    {
        global $xoopsDB;
        $sql = 'SELECT * FROM ' . $xoopsDB->prefix('xmartin_room_price') . " WHERE room_id = $room_id and room_date = $room_date ";
    ...
    public function getMaxDate($mouth)
    {
        ...
        $sql     = 'SELECT max(room_date) FROM ' . $xoopsDB->prefix('xmartin_room_price') . " WHERE room_date < $maxdate and room_date > $date ";
        $result  = $this->db->query($sql);
        if (!$result) {
            return 0;
        }
    ...
    public function getHotelRoom($hotel_id)
    {
        ...
        $roomBedTypeList = getModuleArray('room_bed_type', 'room_type', true);
        $sql             = "SELECT r.*,rt.room_type_info,rp.*,
            ...
            FROM {$xoopsDB->prefix('xmartin_room')} r
            INNER JOIN {$xoopsDB->prefix('xmartin_room_type')} rt ON (rt.room_type_id = r.room_type_id) ";
        $sql             .= "INNER JOIN {$xoopsDB->prefix('xmartin_room_price')} rp ON (r.room_id = rp.room_id) WHERE ";
        $sql             .= $this->check_date ? "rp.room_date BETWEEN {$this->check_date[0]} AND {$this->check_date[1]} " : "rp.room_date = $time ";
        $sql             .= "AND r.hotel_id = $hotel_id AND r.room_status = 1 GROUP BY r.room_id room BY r.room_sented_coupon DESC , r.room_id DESC ";
        //echo $sql;
        $rows   = [];
        $result = $xoopsDB->query($sql);
        while (false !== ($row = $xoopsDB->fetchArray($result))) {
    ...
    public function getRoomDatePrie($room_id, $check_in_date, $check_out_date)
    {
        ...
        $sql = 'SELECT * FROM ' . $xoopsDB->prefix('xmartin_room_price') . " WHERE room_id = $room_id AND room_date BETWEEN $check_in_date AND $check_out_date ";
