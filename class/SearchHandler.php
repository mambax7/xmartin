 ...
 * Class SearchHandler
 */
class SearchHandler extends \XoopsPersistableObjectHandler
{
    /**
     * @return Search
     */
    ...
    public function search($data)
    {
        ...
        //var_dump($data);
        $sql           = 'SELECT h.*,hc.city_name AS hotel_city FROM ' . $this->db->prefix('xmartin_hotel') . ' h ';
        $sql           .= 'INNER JOIN ' . $this->db->prefix('xmartin_room') . ' r ON (h.hotel_id = r.hotel_id) ';
        $sql           .= 'INNER JOIN ' . $this->db->prefix('xmartin_room_price') . ' rp ON (r.room_id = rp.room_id) ';
        $sql           .= 'INNER JOIN ' . $this->db->prefix('xmartin_hotel_city') . ' hc ON (h.hotel_city = hc.city_id) ';
        $sql           .= ' WHERE 1 = 1 ';
        $sql           .= (empty($hotel_address) || empty($hotel_ids)) ? '' : $city_ids . ' IN h.hotel_city_id ';
        $sql           .= empty($hotel_name) ? '' : "AND h.hotel_name LIKE '%$hotel_name%' ";
        $sql           .= $city_id > 0 ? 'AND h.hotel_city IN (SELECT city_id FROM ' . $this->db->prefix('xmartin_hotel_city') . " WHERE city_parentid = $city_id ) " : '';
    ...
    {
        global $xoopsDB;
        $sql    = 'SELECT city_id FROM ' . $xoopsDB->prefix('xmartin_hotel_city') . " WHERE city_name LIKE '%$city_name%'";
        $result = $xoopsDB->query($sql);
    ...
    public function getHotelRooms($room_date)
    {
        ...
        $sql    = 'SELECT r.*,rt.room_type_info,rp.room_is_today_special,rp.room_date,
            ...
            FROM ' . $this->db->prefix('xmartin_room') . ' r INNER JOIN ' . $this->db->prefix('xmartin_room_type') . ' rt ON (r.room_type_id = rt.room_type_id)
            INNER JOIN ' . $this->db->prefix('xmartin_room_price') . ' rp ON (rp.room_id = r.room_id)
    ...
    public function getCityName($city_id)
    {
        ...
        $sql = 'SELECT city_name FROM ' . $this->db->prefix('xmartin_hotel_city') . " WHERE city_id = $city_id";
