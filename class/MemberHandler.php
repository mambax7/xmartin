 ...
 * @author    Martin <china.codehome@gmail.com>
 * */
class MemberHandler extends \XoopsPersistableObjectHandler
{
    /**
     * @create    cart object
     * @license   http://www.blags.org/
    ...
    {
        $sql = "INSERT INTO {$this->db->prefix('xmartin_user_favorite')} (uid,hotel_id) VALUES ($uid,$hotel_id)";
    ...
    public function getHotelList($start, $isLived = false)
    {
        ...
        $uid = $xoopsUser->uid();
        if ($isLived) {
            $order_table = $xoopsDB->prefix('xmartin_order');
            $sql_str     = "SELECT r.hotel_id FROM {$xoopsDB->prefix('xmartin_room')} r WHERE r.room_id IN
                (
                 SELECT oqr.room_id FROM {$xoopsDB->prefix('xmartin_order_query_room')} oqr
                 WHERE order_id IN
                 (SELECT o.order_id FROM $order_table o WHERE o.order_uid = $uid AND o.order_status = 14)
                 UNION
                 SELECT mor.room_id FROM {$xoopsDB->prefix('xmartin_order_room')} mor
                 WHERE order_id IN
                 (SELECT o.order_id FROM $order_table o WHERE o.order_uid = $uid AND o.order_status = 14)
                )";
        } else {
            $sql_str = "SELECT f.hotel_id FROM {$xoopsDB->prefix('xmartin_user_favorite')} f WHERE f.uid = $uid";
        }
        ...
        $sql = "SELECT h.hotel_id,h.hotel_alias,h.hotel_city,h.hotel_name,h.hotel_city_id
                FROM {$xoopsDB->prefix('xmartin_hotel')} h WHERE h.hotel_id IN (%s) ";
    ...
    public function getCouponList($start)
    {
        ...
        $table = $xoopsDB->prefix('xmartin_user_coupon');
        $uid   = $xoopsUser->uid();
        $sql   = "SELECT count(*) as count FROM $table WHERE uid = $uid";
        list($rows['count']) = $xoopsDB->fetchRow($xoopsDB->query($sql_count));

        $sql  = "SELECT c.*,h.hotel_name FROM $table c
                LEFT JOIN {$xoopsDB->prefix('xmartin_order')} o ON (c.relation_id = o.order_id)
                LEFT JOIN {$xoopsDB->prefix('xmartin_order_room')} mor ON (o.order_id = mor.order_id)
                LEFT JOIN {$xoopsDB->prefix('xmartin_room')} r ON (r.room_id = mor.room_id)
                LEFT JOIN {$xoopsDB->prefix('xmartin_hotel')} h ON (r.hotel_id = h.hotel_id)
             ";
    ...
    public function getOrderHotels($order_ids)
    {
        ...
        $sql = "SELECT h.hotel_name,h.hotel_alias,h.hotel_city,mor.order_id,rt.room_type_info FROM {$xoopsDB->prefix('xmartin_hotel')} h ";
        $sql .= "LEFT JOIN {$xoopsDB->prefix('xmartin_room')} r ON (r.hotel_id = h.hotel_id) ";
        $sql .= "LEFT JOIN {$xoopsDB->prefix('xmartin_room_type')} rt ON (rt.room_type_id = r.room_type_id) ";
        $sql .= "LEFT JOIN {$xoopsDB->prefix('xmartin_order_room')} mor ON (mor.room_id = r.room_id) ";
        $sql .= 'WHERE mor.order_id IN  (' . implode(',', $order_ids) . ') GROUP BY mor.order_id ';
        $sql .= "UNION SELECT h.hotel_name,h.hotel_alias,h.hotel_city,oqr.order_id,rt.room_type_info FROM {$xoopsDB->prefix('xmartin_hotel')} h ";
        $sql .= "LEFT JOIN {$xoopsDB->prefix('xmartin_room')} r ON (r.hotel_id = h.hotel_id) ";
        $sql .= "LEFT JOIN {$xoopsDB->prefix('xmartin_room_type')} rt ON (rt.room_type_id = r.room_type_id) ";
        $sql .= "LEFT JOIN {$xoopsDB->prefix('xmartin_order_query_room')} oqr ON (oqr.room_id = r.room_id) ";
    ...
    {
        global $xoopsDB;
        $sql = "SELECT os.*,s.*,st.*,hs.service_extra_price FROM {$xoopsDB->prefix('xmartin_hotel_service')} s ";
        $sql .= "INNER JOIN {$xoopsDB->prefix('xmartin_order_service')} os ON (os.service_id = s.service_id) ";
        $sql .= "INNER JOIN {$xoopsDB->prefix('xmartin_hotel_service_type')} st ON (st.service_type_id = s.service_type_id) ";
        $sql .= "INNER JOIN {$xoopsDB->prefix('xmartin_hotel_service_relation')} hs ON (s.service_id = hs.service_id) ";
